...
const MovimientosScreen = () => {
  ...
  const renderItem = useCallback(({ item }) => {
    ...
    return (
      <View style={movimientoStyles.tarjetaMovimiento}>

        <View style={[movimientoStyles.iconoMovimiento, { backgroundColor: esGasto ? "rgba(255, 75, 75, 0.1)" : "rgba(52, 199, 89, 0.1)" }]}>
          <Text style={{ fontSize: 20 }}>{esGasto ? "⬇️" : "⬆️"}</Text>
        </View>

        <View style={{ flex: 1 }}>
          <Text style={movimientoStyles.movimientoDesc} numberOfLines={1}>{item.descripcion}</Text>

          <View style={movimientoStyles.movimientoFechaContainer}>
            <Text style={movimientoStyles.movimientoFechaIcon}>📅</Text>
            <Text style={movimientoStyles.movimientoFecha}>
        ...
        <View style={{ alignItems: "flex-end" }}>
          <Text style={[movimientoStyles.movimientoMonto, { color: esGasto ? "#ff4b4b" : "#34c759" }]}>
            {esGasto ? "-" : "+"} {item.simboloDivisa}{(item.monto || 0).toLocaleString("es-AR", { minimumFractionDigits: 2 })}
          </Text>
          <Text style={movimientoStyles.movimientoTipo}>
  ...
  if (cargando) {
    return (
      <View style={movimientoStyles.centroTotal}>
  ...
  return (
    <View style={movimientoStyles.contenedorPrincipal}>
      <View style={movimientoStyles.seccionEncabezado}>
        <Text style={movimientoStyles.tituloPrincipal}>Historial General</Text>
        <Text style={movimientoStyles.descripcionEncabezado}>Filtrado detallado de todas sus transacciones.</Text>
      </View>

      <View style={movimientoStyles.buscadorContenedor}>
        <Text style={movimientoStyles.buscadorIcono}>🔍</Text>
        <TextInput
          style={movimientoStyles.buscadorInput}
      ...
      {datosFiltrados.length === 0 ? (
        <View style={movimientoStyles.avisoVacio}>
          <Text style={movimientoStyles.mensajeVacio}>No se encontraron transacciones.</Text>
        </View>
      ) : (
        <FlatList
          data={datosFiltrados}
